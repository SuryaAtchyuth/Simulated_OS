...
void advance_clock ()
{ CPU.numCycles++;
  //ADDED TO INCREENT TQWAITED IN MLFQ IMPLEMENTATION
   check_wait_time();
...
{ struct eventNode *event;
  ...
  { event = eventHead;
    ...
    { case actTQinterrupt:
        ...
        //MLFQ IMPLEMENTATION FOR LEVEL DOWN AFTER TQ EXPIRATION
      ...
      case actReadyInterrupt:
        ...
        set_interrupt (endIOinterrupt);
         fprintf(stdout,"IOinterrupt set  -----------time:%d",CPU.numCycles);
        break;
      case actNull:
        if (clockDebug)
